...
import { getInvites } from 'apiSdk/invites';
import { InviteInterface } from 'interfaces/invite';

import { InviteListPage as ShowCaseList } from 'pages/invites';
...
function DashboardPage() {
  ...
  const inviteStat = useSWR<PaginatedInterface<InviteInterface>>(
    () => `/invites`,
    () => getInvites({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Invites',
      result: inviteStat?.data?.totalCount,
      isLoading: inviteStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`invites`} passHref legacyBehavior>
                  <Button
